...
use crate::{
    coord, Coord, CoordNum, Geometry, GeometryCollection, Line, LineString, MultiLineString,
...
mod test {
    ...
    fn create_point() -> (Point<f64>, Vec<Coord<f64>>) {
        (point!(x: 1., y: 2.), vec![coord! { x: 1., y: 2. }])
    }

    fn create_triangle() -> (Triangle<f64>, Vec<Coord<f64>>) {
        (
            Triangle::new(
                coord! { x: 1., y: 2. },
                coord! { x: 3., y: 4. },
    ...
    fn create_rect() -> (Rect<f64>, Vec<Coord<f64>>) {
        (
    ...
    fn create_line_string() -> (LineString<f64>, Vec<Coord<f64>>) {
        (
            line_string![
                (x: 1., y: 2.),
                (x: 2., y: 3.),
    ...
    fn create_polygon() -> (Polygon<f64>, Vec<Coord<f64>>) {
        (
            polygon!(
